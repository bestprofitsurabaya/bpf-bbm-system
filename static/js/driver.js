...
async function deleteCash(id){if(!confirm('Hapus pengajuan ini?'))return;try{var sel=document.getElementById('kasbon_driver');var driver=sel&&sel.options[sel.selectedIndex]?.value||'';await fetch('/api/cash/delete/'+id+(driver?'?driver='+encodeURIComponent(driver):''),{method:'POST'});loadKasbonHistory();loadPendingLPJ();}catch(e){}}
